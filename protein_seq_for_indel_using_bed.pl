...
my $out_dir=$dir_vcf[0]; 

for(my $i=1;$i<scalar @dir_vcf -1;$i++)
 {
   $out_dir=join("/",$out_dir,$dir_vcf[$i]); 
 }

if(!(-e $filename_vcf_somatic)) { print "error indel running\n"; exit; }
	...
	if (open (IN,"$filename_vcf_somatic"))
	{
		...
		while ($line=<IN>)
		{
			...
			if ($line=~/^([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)\t([^\t]*)/)
			{
				...
				$pos--;
				$new=~s/\,.*$//; #####
				print "pos=$pos","\t","id=$id","\t","old=$old", "\t", "new=$new","\t","qaul=$qaul","\t","anno=$anno","\t","type=$type","\n"; 
				#<STDIN>;
